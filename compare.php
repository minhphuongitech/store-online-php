...
<?php 
	...
	if(isset($_GET['delid']) && $_GET['delid'] != NULL) {
		$deleteProdCompare = $product->deleteProductCompare($_GET['delid'], $cusId);
	}

	if($cusId == NULL) {
		echo "<script>window.location = 'login.php'</script>";
	} else {
		$prodCompare = $product->getProductCompareByCustomer($cusId);
	}
?>
    ...
    <div class="content">
    	<div class="section group">
    		<div class="content_top">
	    		<div class="heading">
	    			<h3><?= _YOUR_COMPARE_PRODUCTS?></h3>
	    			<?php 
	    				if(isset($deleteProdCompare)) {
	    					echo $deleteProdCompare;
	    				}
	    			?>
	    		</div>
    		<div class="clear"></div>
			    	...
			    	<h2 style="font-size: 22px;"><?= _PRODUCTS_LIST?></h2>
						<table class="tblone">
							...
							<?php 
								if($prodCompare) {
									$i = 0;
									while ($result = $prodCompare->fetch_assoc()) {
										$i++;
							?>
							<tr>
								<td><?php echo $i ?></td>
							...
							<?php 
									...
									$alert = "<span class='error'>Your compare products list is empty.</span>";
								}
							?>
						</table>
						<?php 
